...
const generateVoucherNumber = async (client, org_id) => {
  const last = await client.query(
    `SELECT voucher_number FROM stock_movements
     WHERE voucher_number IS NOT NULL AND org_id = $1
...
const createBulkBatches = async (req, res, next) => {
  try {
    ...
    try {
      ...
      // Use existing voucher_number if provided (appending to an existing MI), otherwise generate new per org
      const voucherNumber = existingVoucherNumber || await generateVoucherNumber(client, org_id);
